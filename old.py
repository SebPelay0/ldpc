...
class LDPCEncoder():
    ...
    def minSumDecode(self, codeword):
        ...
        for index in range(len(codeword)):
            sigma2 = 1 / (2 * (10**(self.SNR / 10)))  
            LLR = codeword[index] / sigma2  # Correct LLR calculation
        ...
        while numIterations < 30:
            ...
            for j in range(int(self.m)):  
                ...
                for target in Ej:
                    ...
                    messageSign = numpy.prod(numpy.sign(bitNodes[excludeTarget]))  
                    message = minLLR * messageSign
                    messagesReceivedByBits[target] +=( message * 0.75)
    ...
    def write(self, filePath, BER, bitEnergyRatio, BERs):
        with open(filePath, "a") as file:
            file.write(f"{time.ctime()}: BER = {BER}, SNR={self.SNR}, Eb/N0={bitEnergyRatio}, BERs={BERs}\n")


#numpy.random.seed(42)       


"""SHORT MESSAGE 1/4 CODE RATE"""
test0 = LDPCEncoder(4,5, 250)
message0 = numpy.random.randint(0, 2, size=124).tolist()  
""""n=200, r = 1/4"""
test1 = LDPCEncoder(30,50, 300)
message1 = numpy.random.randint(0, 2, size=87).tolist()  
...
# test0.encode(message0)
# noisyCodeword = test0.addNoiseBPSK(5, True)
# print(test0.minSumDecode(noisyCodeword))
...
def plot():
    ...
    for snr in snrRange:
        ...
        BERs  = []
        for it in range(n):
            message0 = numpy.random.randint(0, 2, size=53).tolist()  
            test0.encode(message0) 
            noisyCodeword = test0.addNoiseBPSK(snr)
            BER = test0.minSumDecode(noisyCodeword)
            BERs.append(BER)
        ...
        test0.write("results1.txt", avgBER/n, test0.bitEnergyRatio, BERs)
